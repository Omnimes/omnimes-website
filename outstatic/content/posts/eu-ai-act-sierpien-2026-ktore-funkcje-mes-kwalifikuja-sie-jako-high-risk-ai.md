...
**3. Monitoring wydajności i zachowania operatorów (Annex III, pkt 4b).** AI Act wprost klasyfikuje jako AI wysokiego ryzyka systemy AI „przeznaczone do monitorowania i oceny wydajności i zachowania osób w stosunkach pracy". Tu wpada większość modułów MES, które:
...
To **najczęściej pomijany** obszar. Polski kierownik produkcji często nie zdaje sobie sprawy, że dashboard z rankingiem operatorów połączony z systemem premiowym może być AI wysokiego ryzyka z perspektywy regulatora.

**4. AI wspierające decyzje kadrowe — przydział zadań, awanse, zwolnienia (Annex III, pkt 4b).** Jeżeli wasz MES rekomenduje przeniesienie operatora na inną linię na podstawie historii błędów, albo wspiera decyzje o redukcji zatrudnienia po analizie produktywności zespołów — to AI wysokiego ryzyka.

**5. AI w zarządzaniu krytyczną infrastrukturą energetyczną, wodną, gazową (Annex III, pkt 2).** Dotyczy zakładów, które są częścią dostaw energii elektrycznej, wody, gazu, ciepła. Jeżeli wasz EMS optymalizuje pracę agregatów kogeneracyjnych zasilających lokalną sieć, AI w warstwie sterowania tym procesem jest AI wysokiego ryzyka.

## Funkcje MES, które prawie na pewno NIE są AI wysokiego ryzyka
...
- **Analiza energii w ramach EMS dla raportowania ESG/ISO 50001** — to zgodność z inną regulacją, nie AI Act.
...
Jeżeli funkcja MES wpada w kategorię wysokiego ryzyka, to provider (czyli wy, jeśli rozwijacie własne moduły AI; lub dostawca, jeśli kupujecie) i deployer (czyli zakład produkcyjny używający systemu) muszą spełnić ([Article 17](https://www.euaiact.com/article/17), [Annex VI](https://artificialintelligenceact.eu/annex/6/), [Article 26](https://artificialintelligenceact.eu/article/26/)):
...
3. **Ocena zgodności (conformity assessment)** — wewnętrzna (Annex VI) lub przez jednostkę notyfikowaną (Annex VII), zależnie od typu systemu i obszaru. Większość systemów Annex III idzie ścieżką wewnętrzną.
4. **Rejestracja w bazie danych UE** — przed wprowadzeniem na rynek system wysokiego ryzyka z Annex III musi być wpisany do publicznej bazy.
...
7. **Logowanie i ślad audytowy** — automatyczne logi wszystkich istotnych zdarzeń, przechowywane przez minimum 6 miesięcy.

Dla deployera (czyli właściciela zakładu używającego MES wysokiego ryzyka) dochodzi jeszcze obowiązek z **Article 26**: **przed wdrożeniem AI wysokiego ryzyka w miejscu pracy musicie poinformować przedstawicieli pracowników i samych pracowników** o tym, że będą poddani działaniu takiego systemu. To nie jest opcjonalne i nie da się tego obejść regulaminem pracy podpisanym dwa lata wcześniej.
...
- Brak krajowych procedur egzekucyjnych nie jest argumentem obronnym — UODO już sygnalizuje, że w obszarze danych osobowych w AI będzie działać na podstawie RODO niezależnie od stanu polskiej ustawy AI.
...
- **Naruszenia obowiązków wysokiego ryzyka** (większość scenariuszy MES): do **15 mln EUR lub 3% obrotu**.
...
Koszt zgodności? Z naszej praktyki w OmniMES: **wewnętrzna ocena zgodności (Annex VI) dla jednego modułu AI to typowo 15–30 dni roboczych zespołu (architekt, prawnik IP, data scientist)**. Jednostka notyfikowana (Annex VII), jeżeli wymagana — 6–12 tygodni i koszty audytu od 25 do 80 tys. EUR. Dla porównania: jedna kara za naruszenie w kategorii wysokiego ryzyka w wysokości 3% obrotu zakładu z przychodem 100 mln EUR to 3 mln EUR. Matematyka mówi sama za siebie.

## Co zrobić w pozostałych ~3 miesiącach — lista kontrolna

Realnie do 2 sierpnia 2026 zostały trzy miesiące. To wystarczy na **inwentaryzację i fundamenty**, nie na pełną ocenę zgodności. Sensowna sekwencja:

1. **Zinwentaryzujcie wszystkie funkcje MES/EMS/CMMS używające AI lub uczenia maszynowego.** Lista powinna obejmować: nazwę funkcji, model (jeśli znany), źródło (wewnętrzny vs dostawca zewnętrzny), opis decyzji, którą AI wspiera lub podejmuje.
...
3. **Dla funkcji oznaczonych jako AI wysokiego ryzyka** zacznijcie od dokumentacji technicznej i logów. To podstawa, na której oprzecie wszystko inne. Bez tego nie macie czego oceniać.
4. **Zorganizujcie spotkanie z przedstawicielami pracowników** w sprawie tych funkcji, które monitorują wydajność/zachowanie. Article 26 wymaga informacji **przed** wdrożeniem — jeżeli system już działa, to konsultacja powinna się odbyć jak najszybciej.
5. **Sprawdźcie kontrakty z dostawcami MES.** Kto jest providerem w rozumieniu AI Act? Czy dostawca zobowiązuje się do oceny zgodności? Czy macie dostęp do dokumentacji technicznej? To są negocjowane teraz, nie po pierwszej kontroli.
...
AI Act nie ma na celu zatrzymania automatyzacji w przemyśle. Ma zapewnić, że automatyzacja, która podejmuje decyzje wpływające na ludzi, jest **przejrzalna, audytowalna i kontrolowana**. Jeżeli OmniMES, SAP DM czy Siemens Opcenter już dziś prowadzi dziennik audytu każdej decyzji, ma RBAC na moduł i pozwala kierownikowi nadrzędować nad rekomendacjami AI — to wymagania AI Act dla większości funkcji są bardziej proceduralne niż technologiczne.

Najgorszy scenariusz: zakład odkrywa we wrześniu 2026, że dashboard z rankingiem operatorów to AI wysokiego ryzyka, którego nie zinwentaryzował, nie udokumentował i nie skonsultował z przedstawicielami pracowników. KRiBSI nie będzie zainteresowane wymówką „myśleliśmy że to tylko Excel z OEE". Zgodność to nie projekt na tydzień przed terminem — to system, który warto budować od teraz.
