...
Niżej rozbieram to konkretnie. Co to TS Foundation Models, jak działają, na czym zostały trenowane, jaki wynik dają na realnych danych z czujników z polskiej fabryki w porównaniu do dobrze postawionego XGBoost-a, jakie są latencje na Jetson Orin i GPU, i kiedy każde z podejść ma sens.
...
## Benchmark na realnych danych z czujników
...
**TimesFM 2.0 zero-shot** — bez fine-tuningu, model widzi 24h kontekst (86400 punktów na sygnał), prognozuje 24h naprzód, wykrywanie anomalii przez porównanie prognozy z rzeczywistością (residual > 3 sigma → alarm).
...
Wniosek jest mniej sensacyjny, niż mógłby być. Zero-shot TimesFM/Chronos łapie 2 z 3 awarii — to przyzwoity wynik bez minuty pracy data scientista, ale gorszy niż własny XGBoost i z 5–7x większą liczbą fałszywych alarmów. Fine-tune TimesFM wyrównuje recall do XGBoost-a, ma trochę więcej FP, i wymaga **2 tygodnie zamiast 3 tygodni** wdrożenia. To realna oszczędność czasu, ale nie rewolucja.

Kluczowy detal: **dla wielu przypadków użycia zero-shot wystarczy.** Predykcja zapotrzebowania energetycznego na linii produkcyjnej, prognoza zużycia surowca, planowanie produkcji na 1 tydzień — wszystko to są zadania, gdzie 66% recall i lekko podwyższone FP są akceptowalne, a brak wdrożenia 3-tygodniowego = projekt rusza w 2 dni zamiast 2 miesięcy.
...
- XGBoost — 4 ms
- TimesFM — 850 ms (tryb CPU, bo to nie jest model zaprojektowany pod urządzenia brzegowe)
- Chronos-Bolt — 420 ms
...
Praktyczna obserwacja: **Chronos-Bolt jest najlepszy do edge'a.** Mniejszy od TimesFM, optymalizowany pod inferencję, T5-based architektura dobrze działa z kwantyzacją INT8. Dla typowego przypadku użycia MES (alarmy co minutę dla 50 maszyn) Jetson Orin NX z Chronos-Bolt obsługuje cały zakład komfortowo — łączna pamięć RAM poniżej 8 GB, łączny budżet opóźnień pod 1s na wszystkie maszyny.

Koszt operacyjny: dla zakładu z 50 maszynami, alarmowanie co 15 minut, jedna predykcja na maszynę na cykl:
...
- musisz osiągnąć metryki SLA dla alarmowania (recall ≥ 95%)
- masz unikalną konfigurację czujników, której nie ma w danych pretreningowych (np. wysokoczęstotliwościowy strumień akustyczny)

**Custom XGBoost wciąż jest lepszy gdy:**
- masz bogatą wiedzę domenową, którą da się zakodować w feature engineering
- wyjaśnialność jest krytyczna (SHAP values dla XGBoost vs „bo tak transformer powiedział")
...
3. Trening, strojenie hiperparametrów (2–4 tyg.)
4. Walidacja domenowa z operatorami (2 tyg.)
5. Wdrożenie i monitoring (1 mies.)

Razem 4–6 miesięcy, ~EUR 30–60k pracy data scientista per linia.

Z TS Foundation Models, dla 60–70% typowych przypadków użycia MES (prognozowanie, wykrywanie anomalii w danych ustabilizowanych), pipeline wygląda inaczej:
1. Zbieranie danych (1–2 mies. — to nie zniknęło)
2. Konfiguracja modelu foundation na urządzeniach brzegowych (1 tydzień)
...
Dla pozostałych 30–40% (predykcja rzadkich awarii, krytyczne alarmowanie, regulowane branże) własny XGBoost lub dostrojony (fine-tuned) foundation model wciąż jest lepszy.

## Bariery i ograniczenia

Lista barier, których zakres tego artykułu wymaga uczciwie poruszyć:

**1. Brak interpretowalności.** XGBoost ma SHAP values, drzewa decyzyjne — operator widzi „alarm bo wibracja X w paśmie 50–80 Hz wzrosła o 40%". Foundation model w trybie zero-shot daje tylko residual — „rzeczywistość odbiega od prognozy". Dla zgodności ([AI Act — AI wysokiego ryzyka](/blog/eu-ai-act-sierpien-2026-ktore-funkcje-mes-kwalifikuja-sie-jako-high-risk-ai)) to istotna luka.

**2. Zajętość pamięci na urządzeniach brzegowych.** TimesFM 2.0 (500M params) zajmuje 4 GB RAM w FP16. To wyklucza Jetson Nano (8 GB), Raspberry Pi, większość starych urządzeń wbudowanych. Trzeba kupować nowy sprzęt (Jetson Orin NX/AGX), co dla zakładu z 50 maszynami i lokalną inferencją na każdej nie jest trywialne.
...
**4. Ryzyko dostawcy.** Google może wycofać TimesFM jak wycofał Translate API. Amazon może zmienić licencję Chronosa (precedens z DeepSeek-V3, gdzie warunki licencji zmieniły się 4 razy w 6 miesięcy). Trzymanie wagi modelu lokalnie pomaga, ale aktualizacje przestają być dostępne.

**5. Jakość pretreningu vs domena MES.** Dane pretreningowe TimesFM-a (Google Trends, Wikipedia views) są bardzo różne od danych z czujników 1 kHz z wibracjami w zakresie 10–500 Hz. Moirai jest lepszy w tym wymiarze (LOTSA zawiera dane techniczne), ale wciąż — to nie jest model wytrenowany na MES.
...
- Chronos-Bolt-Large na Jetson Orin NX dla wszystkich „łatwych" zadań: prognozowanie zużycia mediów, planowanie produkcji, wykrywanie anomalii w danych ustabilizowanych
- Wdrożenie zero-shot, walidacja przez 4 tygodnie obserwacji
- 60–70% przypadków użycia pokryte w 2–3 miesiące

**Warstwa 2 — custom dla krytycznych:**
- XGBoost lub dostrojony (fine-tuned) TimesFM dla rzadkich, krytycznych awarii
- Bogata wiedza domenowa, SHAP values dla zgodności
- 4–6 miesięcy na przypadek użycia, ale tylko 30% najważniejszych
...
Dla zakładu zaczynającego dziś — startujcie od warstwy 1 z Chronos-Bolt. Szybkie zwycięstwo w 6 tygodni, daje 60% wartości i pokazuje organizacji, że ML działa. Potem dorabiacie warstwę 2 dla rzeczy, które warstwa 1 nie umie.

## Wnioski dla dyrektora produkcji i lead data scientist

Trzy konkrety:

**Po pierwsze**, TS Foundation Models nie są jeszcze rewolucyjne dla MES, ale są ewolucyjne dla 60–70% typowych zadań. Pierwszy raz w historii ML można dostać sensowną prognozę zużycia mediów albo planowanie produkcji bez 3-miesięcznego projektu. To zmienia kalkulację „czy w ogóle warto" dla mniejszych zakładów.

**Po drugie**, własny XGBoost wciąż jest lepszy dla krytycznych predykcji rzadkich awarii i tam, gdzie potrzebna jest pełna wyjaśnialność dla zgodności. Nie wymieniajcie wszystkiego naraz — hybryda jest bezpieczniejsza i tańsza.

**Po trzecie**, ekosystem dojrzeje. W połowie 2026 widzimy pierwszą falę produkcyjnych wdrożeń. Do 2027 spodziewam się: dedykowanych foundation models dla danych przemysłowych (Volkswagen Industrial Cloud, Siemens MindSphere prawdopodobnie wyjdą z czymś), lepszych narzędzi MLOps dla TS Foundation Models, i wreszcie — fundamentalnych narzędzi interpretacji, których dziś brakuje.

Jeśli macie pipeline XGBoost dla predykcji awarii, który działa — nie wyrzucajcie go. Jeśli planujecie pierwsze wdrożenie ML w MES dla prognozowania mediów albo planowania produkcji — Chronos-Bolt na Jetson Orin to dziś sensowniejszy start niż klasyczny pipeline ML.
