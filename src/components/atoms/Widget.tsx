...
export const Widget = ({ text, href, buttonText }: WidgetProps) => {
  ...
  const localePrefixRegex =
    /^\/(de|pl|en)(\/(?!about$|ueber-uns$|o-nas$|kontakt$|contact$|oferta$|offer$|angebot$|demo$|login$|register$|rejestracja$|admin$|dashboard$|dashboard\/(?:[^\/]+)$|dashboard\/(?:[^\/]+)\/(?:[^\/]+)$|gallery$|galeria$|galerie$|outstatic$|outstatic\/(?:[^\/]+)$).*)?$/
