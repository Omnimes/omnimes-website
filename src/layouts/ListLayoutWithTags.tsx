...
export default function ListLayoutWithTags({ posts, tags, tag, pagination }: ListLayoutProps) {
          ...
          <div className="hidden h-full max-h-screen min-w-[280px] max-w-[280px] flex-wrap overflow-auto rounded bg-gray-50 pt-5 shadow-md sm:flex dark:bg-gray-900/70 dark:shadow-gray-800/40">
            <div className="px-6 py-4">
              {pathname?.startsWith("/blog") ? (
                <h3 className="text-primary-500 font-bold uppercase">{t("allPost")}</h3>
              ) : (
                <CustomLink
                  href={`/blog`}
                    ...
                    <li key={tag.value} className="my-3">
                      {pathname && decodeURI(pathname.split("/tags/")[1]) === slug(tag.label) ? (
                        ...
                        <CustomLink
                          ...
                          className="hover:text-primary-500 dark:hover:text-primary-500 py-2 text-sm font-medium uppercase text-gray-500 dark:text-gray-300"
