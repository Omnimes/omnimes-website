...
async function getData(locale: string) {
  ...
  const allCourses = await db
    .find<ExtendedOstDocument>(
      { collection: "courses", status: "published", lang: locale },
  ...
  const total = getDocuments("courses", ["lang"])
    .filter((doc) => doc.status === "published" && doc.lang === locale).length;
...
async function getDataToSearch() {
  ...
  const docs = AllCourses.map((d) => ({
    ...d,
    tags: d.tags.map((t) => t.label).join(", "),
  }));

  // jeśli chcesz osobny plik pod wyszukiwarkę kursów,
  // rozważ dodanie opcjonalnego parametru typu: generateSearchJSON(docs, "courses")
